...
class CamembertEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0, embed_position=True
    ):
        if embed_position:
...
class CamembertSelfAttention_label(nn.Module):
    ...
    def forward(
        ...
        #print('query :', query_layer.size(), 'key :', key_layer.transpose(-1, -2).size())
        ...
        if label_embeddings is not None:
            sentences_length = hidden_states.size()[1]
            mixed_lb_query = self.query(label_embeddings) #batch_size * num_labels * all_head_size
            #transposing before multi-head product
            lb_query = self.transpose_for_scores(mixed_lb_query) #batch_size * num_head * num_labels * head_size

            lb_attention_scores = torch.matmul(lb_query, key_layer.transpose(-1, -2)) #batch_size * num_heads * num_labels
                                                                                      # * sentence_length (including [CLS])
            lb_attention_scores = lb_attention_scores / math.sqrt(self.attention_head_size)

            ## Computing CoSim
            lb_attention_scores_normed = nn.functional.normalize(lb_attention_scores, p=2.0, dim=2)

            new_attention = torch.matmul(lb_attention_scores_normed.transpose(2, 3),
                                         lb_attention_scores_normed,
                                         )
            new_attention = new_attention - torch.eye(sentences_length, device=hidden_states.device) #removing the obvious similarity
                
            attention_scores = attention_scores + new_attention #updating the attentions

        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in CamembertModel forward() function)
            attention_scores = attention_scores + attention_mask
            if label_embeddings is not None:
                lb_attention_scores = lb_attention_scores + attention_mask
        ...
        if label_embeddings is not None:
            # Normalize the label attention scores to probabilities.
            lb_attention_probs = nn.functional.softmax(lb_attention_scores, dim=-1)
            lb_attention_probs = self.dropout(lb_attention_probs)

            # Mask heads if we want to
            if head_mask is not None:
                lb_attention_probs = lb_attention_probs * head_mask

            lb_context_layer = torch.matmul(lb_attention_probs, value_layer)# batch_size * num_heads * num_labels * head_size
            lb_context_layer = lb_context_layer.permute(0, 2, 1, 3).contiguous()

            new_lb_context_layer_shape = lb_context_layer.size()[:-2] + (self.all_head_size,)
            lb_context_layer = lb_context_layer.view(new_lb_context_layer_shape)

            outputs = ((context_layer, lb_context_layer), (attention_probs, attention_scores, new_attention, lb_attention_scores_normed)) if output_attentions else ((context_layer, lb_context_layer), )
...
class CamembertAttention_label(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = CamembertSelfAttention_label(config, position_embedding_type=position_embedding_type)
    ...
    def forward(
        ...
        #lb_value_layer = self.self.value(label_embeddings)
        if label_embeddings is not None:
            lb_attention_output = self.output(self_outputs[0][1], label_embeddings)
            outputs = ((attention_output, lb_attention_output), ) + self_outputs[1:]  # add attentions if we output them
        return outputs
...
class CamembertLayer_label(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = CamembertAttention_label(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = CamembertAttention_label(config, position_embedding_type="absolute")
    ...
    def forward(
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            attention_output = cross_attention_outputs[0][0]
        ...
        if label_embeddings is not None:    
            lb_layer_output = apply_chunking_to_forward(
                self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, self_attention_outputs[0][1]
            )
            outputs = ((layer_output, lb_layer_output), ) + outputs
...
class CamembertEncoder_label(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([CamembertLayer_label(config) for i in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        all_hidden_states = () if output_hidden_states else None #stores couples (hidden_state, label_embeddings)
        ...
        for i, layer_module in enumerate(self.layer):
            if output_hidden_states:
                if label_embeddings is not None:    
                    all_hidden_states = all_hidden_states + ((hidden_states, label_embeddings),)
                else:
            ...
            if label_embeddings is not None:
                label_embeddings = layer_outputs[0][1]
        ...
        if label_embeddings is not None:
            ...
            if not return_dict:
                return tuple(
                    v
                    for v in [
                        (hidden_states, label_embeddings),
                        next_decoder_cache,
                        all_hidden_states,
                        all_self_attentions,
            ...
            return BaseModelOutputWithPastAndCrossAttentions(
                last_hidden_state=(hidden_states, label_embeddings),
...
class CamembertPreTrainedModel_label(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, CamembertEncoder_label):
...
class CamembertModel_label(CamembertPreTrainedModel_label):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = CamembertEncoder_label(config)
    ...
    def forward(
        ...
        # embeddings of the labels key_words: shape [num_labels x num_key_words_per_label]
        if label_inputs is not None:
            label_embeddings = self.embeddings(
                input_ids=label_inputs,
                embed_position = False
            )
            label_embeddings = label_embeddings.mean(dim=1)
            #let's repeat the embeddings so that its dimension matches embedding_output
            new_label_embeddings_shape = (1, ) + label_embeddings.shape
            label_embeddings = label_embeddings.view(new_label_embeddings_shape)
            label_embeddings = label_embeddings.repeat(batch_size, 1, 1)
        else:
...
class CamembertForMaskedLM_label(CamembertPreTrainedModel_label):
    ...
    def __init__(self, config):
        ...
        self.roberta = CamembertModel_label(config, add_pooling_layer=False)
    ...
    def forward(
        ...
        label_inputs: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
...
class CamembertForSequenceClassification_label(CamembertPreTrainedModel_label):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class CamembertForMultipleChoice(CamembertPreTrainedModel_label):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config):
        super().__init__(config)

        self.roberta = CamembertModel_label(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class CamembertForTokenClassification_label(CamembertPreTrainedModel_label):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def forward(
        ...
        sequence_output = tuple(self.dropout(v) for v in sequence_output)
        logits = self.layer_norm(sequence_output[0])
...
class CamembertForQuestionAnswering(CamembertPreTrainedModel_label):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class CamembertForCausalLM(CamembertPreTrainedModel_label):
    _keys_to_ignore_on_save = [r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_missing = [r"position_ids", r"lm_head.decoder.weight", r"lm_head.decoder.bias"]
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
